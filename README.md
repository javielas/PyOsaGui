...
To adapt to any other OSA, just create a new class in a python script with the relevant functions of the OSA.
...
## Usage
You set the start and stop wavelength, sensitivity, reference level, resolution, and points/nm for the first sweep. You cannot change these values once you sweep and get a trace. This is to ensure that all the traces that are saved in the same file have the same configuration values. In order to change them, you can create a new group and sweep again. Each group has to be saved separately.
...
A spectrum trace can be deleted by selecting (not checking) a single trace(it would be highlighted in the list), and then clicking the delete button. Entire groups can also be deleted. It always asks for confirmation before deleting.

The traces can be set as visible or invisible with the corresponding checkbox in the list. The names of the traces can be changed by double-clicking its name. The save button saves only the checked(visible) traces. It can be saved in csv format or NetCDF. NetCDF is the preferred format as it holds all the information of the configuration parameters and units and is easier to handle for data processing.
